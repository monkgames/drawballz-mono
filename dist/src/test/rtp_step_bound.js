...
function measureRTP(seed, table, N, randomizePlayers) {
    const req = {
...
function scaleTable(table, step) {
    return {
...
async function main() {
    ...
    for (const requested of [1.2, 0.8, 1.05, 0.95]) {
        ...
        if (jump > 1.1) {
            throw new Error(`Step ${requested} caused RTP jump >±1.1% after clamp (jump=${jump.toFixed(2)}%)`);
        }
    }
