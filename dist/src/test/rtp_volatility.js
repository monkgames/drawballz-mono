...
function measureRTP(seed, table, N, betMin = 1, betMax = 1, randomizeBet = false) {
    const req = {
...
async function main() {
    ...
    for (const s of seeds.slice(1)) {
        ...
        if (diff > 1.0) {
            throw new Error(`Seed volatility exceeded ±1% (diff=${diff.toFixed(2)}%)`);
        }
    }
    ...
    if (runDiff > 1.0) {
        throw new Error(`Run-to-run volatility exceeded ±1% (diff=${runDiff.toFixed(2)}%)`);
    }
    // eslint-disable-next-line no-console
