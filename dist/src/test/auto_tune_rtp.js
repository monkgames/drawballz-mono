...
function rtpForTable(seed, table, N, betMin = 1, betMax = 10) {
    const req = {
...
async function calibrate(targetPct) {
...
async function main() {
    ...
    for (const t of targets) {
        ...
        if (err > 0.01) {
            throw new Error(`Auto-tune failed to achieve target RTP=${t}% (err=${(err * 100).toFixed(2)}%, iterations=${iterations})`);
        }
        // eslint-disable-next-line no-console
        console.log(`OK: auto-tune achieved RTP≈${(after * 100).toFixed(2)}% for target ${t}% in ${iterations} iterations`);
    }
}
main().catch(e => {
    // eslint-disable-next-line no-console
