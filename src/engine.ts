...
function validatePlayerBalls(
	...
	const issues: string[] = []
	if (balls.length !== 5) issues.push('player must have exactly 5 balls')
	const colorSet = new Set<Color>()
	for (const b of balls) {
		if (b.number < numMin || b.number > numMax)
			issues.push(`invalid number ${b.number}`)
		if (b.color < 1 || b.color > 5) issues.push(`invalid color ${b.color}`)
		if (colorSet.has(b.color)) issues.push(`duplicate color ${b.color}`)
...
function sampleK(dist: Record<number, number>, r: () => number): number {
...
function sampleMask(
	...
	if (kClamped < 1) kClamped = 1
	const colors = [1 as Color, 2 as Color, 3 as Color, 4 as Color, 5 as Color]
...
export function evaluateMatch(input: MatchInput): Outcome {
	...
	const maskCap =
		typeof epoch.maxMaskSize === 'number'
	...
	function sumNums(arr: ReadonlyArray<Ball>): number {
		let s = 0
		for (const b of arr) s += b.number
		return s
	}
	...
	const winningMask = sampleMask(
		maskSeed,
		epoch.maskSizeDistribution,
		numMin,
		numMax,
		maskCap
	)
	...
	for (const w of winningMask) {
		if (setRA.has(`${w.number}-${w.color}`)) mA++
		if (setRB.has(`${w.number}-${w.color}`)) mB++
	}
	const m = mA + mB
	const basePrizeA = epoch.fixedPrizeTable[mA] ?? 0
	const basePrizeB = epoch.fixedPrizeTable[mB] ?? 0
	const betA =
		typeof input.playerA.betAmount === 'number' &&
		input.playerA.betAmount > 0
			? input.playerA.betAmount
			: 1
	const betB =
		typeof input.playerB.betAmount === 'number' &&
...
export function evaluateBatch(req: BatchRequest): BatchResult {
	const outcomes: Outcome[] = []
	let totalPrize = 0
	let totalM = 0
	const distributionM: Record<number, number> = {
		0: 0,
		1: 0,
		2: 0,
		3: 0,
	...
	const matchesPerColor: Record<Color, number> = {
		1: 0,
		2: 0,
		3: 0,
		4: 0,
	...
	const colorPresence: Record<Color, number> = {
		1: 0,
		2: 0,
		3: 0,
		4: 0,
	...
	const maskColorFrequency: Record<Color, number> = {
		1: 0,
		2: 0,
		3: 0,
		4: 0,
	...
	const cancellationsPerColor: Record<Color, number> = {
		1: 0,
		2: 0,
		3: 0,
		4: 0,
	...
	for (const mInput of req.matches) {
		...
		totalM += out.m
		const betA =
			typeof mInput.playerA.betAmount === 'number' &&
			mInput.playerA.betAmount > 0
				? mInput.playerA.betAmount
				: 1
		const betB =
			typeof mInput.playerB.betAmount === 'number' &&
		...
		for (const c of [1, 2, 3, 4, 5] as Color[]) {
			if (
				out.remainingA.some(b => b.color === c) ||
		...
		for (const w of out.winningMask) {
			...
			if (
				out.remainingA.some(
					b => b.number === w.number && b.color === w.color
				) ||
				out.remainingB.some(
					b => b.number === w.number && b.color === w.color
				)
...
export function evaluateBatchCompact(req: CompactBatchRequest): BatchResult {
	const outcomes: Outcome[] = []
	let totalPrize = 0
	let totalM = 0
	let totalBetRevenue = 0
	...
	const sampleMax =
		typeof req.sampleOutcomes === 'number' && req.sampleOutcomes > 0
			? req.sampleOutcomes
			: 50
	const base =
		typeof req.offset === 'number' && req.offset > 0 ? req.offset : 0
	for (let i = 0; i < req.N; i++) {
		...
		const numMax = 9
		const betMin =
			typeof req.betMin === 'number' && req.betMin > 0 ? req.betMin : 1
		const betMax =
			typeof req.betMax === 'number' && req.betMax >= betMin
				? req.betMax
				: Math.max(betMin, 10)
		const playerA = rnd
			? randomPlayerFromSeed(
					'A',
					req.epoch.seed + ':A:' + idx,
					numMin,
					numMax
			  )
			: req.playerA
		...
		const playerAWithBet: PlayerConfig = {
			...playerA,
			betAmount: betA,
		}
		const playerBWithBet: PlayerConfig = {
			...playerB,
			betAmount: betB,
		}
		const mInput: MatchInput = {
			epoch: req.epoch,
		...
		const out = evaluateMatch(mInput)
		if (outcomes.length < sampleMax) outcomes.push(out)
		totalPrize += out.prize
		totalM += out.m
		distributionM[out.m] = (distributionM[out.m] ?? 0) + 1
		const effectiveBet = (betA + betB) / 2
