...
import { CompactBatchRequest, EpochConfig, MatchInput, PlayerConfig } from '../types'

function mkEpochNoCap(seed: string): EpochConfig {
  return {
    maskSizeDistribution: { 0: 0.0, 1: 0.2, 2: 0.2, 3: 0.2, 4: 0.2, 5: 0.2 },
...
function countMaskSizes(
  outcomes: ReadonlyArray<{ winningMask: ReadonlyArray<{ number: number; color: number }> }>
) {
...
async function testEvaluateMatchDefaultCap5() {
  ...
  for (const seed of ['def-cap-a', 'def-cap-b', 'def-cap-c']) {
    ...
    if (out.winningMask.length < 1) {
      throw new Error(`Default cap must still produce non-empty mask; got ${out.winningMask.length}`)
    }
    if (out.winningMask.length > 5) {
      throw new Error(`Default cap failed: expected ≤5, got ${out.winningMask.length}`)
    }
  }
...
async function testEvaluateBatchDefaultCap5() {
  ...
  if ((maskDist[0] ?? 0) !== 0) {
    throw new Error(`Masks must be non-empty even with default cap; saw size-0=${maskDist[0]}`)
  }
  // eslint-disable-next-line no-console
  console.log('OK: evaluateBatchCompact respects default maxMaskSize=5 when omitted')
}
