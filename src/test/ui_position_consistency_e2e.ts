...
async function main() {
	...
	try {
		...
		const baseline = await page.evaluate(() => {
			...
			return st.slots.map((s: any) => ({ x: s.posX, y: s.posY }))
		})
		...
		// ensure positions per slot remain constant (exact match expected)
		for (let i = 0; i < baseline.length; i++) {
			const a = baseline[i]
			const b = after[i]
			if (a.x !== b.x || a.y !== b.y) {
				throw new Error(
					`slot ${i} position changed: baseline=(${a.x},${a.y}) after=(${b.x},${b.y})`
				)
			}
		}
		await browser.close()
		console.log('UI position consistency e2e passed')
	} finally {
		try {
			preview?.kill('SIGINT')
		} catch (_) {}
...
main().catch(err => {
	console.error('UI position consistency e2e failed:', err)
	process.exit(1)
})
