...
function mkFixedPlayer(id: string, bet: number): PlayerConfig {
	return {
		id,
		balls: [
			{ number: 0, color: 1 },
...
function countMaskSizesFromRows(rows: ReadonlyArray<{ ms: number }>) {
...
async function main() {
	...
	for (const k of [0, 1, 2, 3, 4, 5]) {
		...
		if (expected !== actual) {
			throw new Error(
				`Mask size filter mismatch (wins-only OFF): ms=${k} expected=${expected} actual=${actual}`
			)
		}
	}
	...
	for (const k of [0, 1, 2, 3, 4, 5]) {
		...
		if (expected !== actual) {
			throw new Error(
				`Mask size filter mismatch (wins-only ON): ms=${k} expected=${expected} actual=${actual}`
			)
		}
	}
	// eslint-disable-next-line no-console
	console.log(
		'OK: mask size filter counts match distributions (wins-only OFF/ON)'
	)
}
