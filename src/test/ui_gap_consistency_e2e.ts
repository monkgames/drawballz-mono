...
async function main() {
	...
	try {
		...
		const baseline = await page.evaluate(() => {
			...
			return st.slots.map((s: any) => s.gapPX)
		})
		// set orange and pink unconfigured
		await page.evaluate(() => {
		...
		// configure pink on former orange slot (index 2) to stress new metrics
		await page.evaluate(() => {
		...
		for (let i = 0; i < baseline.length; i++) {
			...
			if (Math.abs(a - b) > tol) {
				throw new Error(
					`slot ${i} trail gap mismatch: baseline=${a}, after=${b}`
				)
			}
		}
		await browser.close()
		console.log('UI trail gap consistency e2e passed')
	} finally {
		try {
			preview?.kill('SIGINT')
		} catch (_) {}
...
main().catch(err => {
	console.error('UI trail gap consistency e2e failed:', err)
	process.exit(1)
})
