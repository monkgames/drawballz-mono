...
function sumCounts(obj: Record<number, number>): number {
	let s = 0
	for (const k of [1, 2, 3, 4, 5]) s += Number(obj[k] || 0)
	return s
}
...
async function testRandomizationAndMetrics() {
	...
	if (result.metrics.count !== N) {
		throw new Error(
			`Count mismatch: expected ${N}, got ${result.metrics.count}`
		)
	}
	...
	if (Math.abs(actualRevenue - expectedRevenue) > tol) {
		throw new Error(
			`Total bet revenue out of expected range: got ${actualRevenue}, expected ~${expectedRevenue}`
		)
	}
	// eslint-disable-next-line no-console
	console.log(
		'OK: randomization produces non-zero color metrics and valid bet revenue'
	)
}
...
async function testDifferentOffsetsProduceDifferentMetrics() {
	...
	const baseReq: Omit<CompactBatchRequest, 'offset' | 'N'> = {
		epoch,
		playerA: mkFixedPlayer('A'),
		playerB: mkFixedPlayer('B'),
		betMin: 1,
	...
	const r0: CompactBatchRequest = { ...baseReq, N, offset: 0 }
	const r1: CompactBatchRequest = { ...baseReq, N, offset: N }
	const a = evaluateBatchCompact(r0).metrics
	const b = evaluateBatchCompact(r1).metrics
	...
	if (aKey === bKey) {
		throw new Error(
			'Metrics identical across different offsets; expected variation due to randomization'
		)
	}
	// eslint-disable-next-line no-console
